...
public class MultiKeyQuicksort<E> extends SortingAlgorithm<E, LexicographicComparator<? super E>> {
    ...
    public void sort(List<E> list, int from, int to, int position) {
        ...
        int c = 0;
        while (i != midto){
            c = comparator.compare(list.get(i), piv, position);
            if (c < 0){ 
                swap(list,midfrom,i);
                midfrom++;
                i++;
        ...
        sort(list, from, midfrom, position);
        sort(list, midfrom, midto, position + 1);
        sort(list, midto, to, position);
    }
    ...
    public static void main(String[] args) {
        MultiKeyQuicksort<String> algorithm = new MultiKeyQuicksort<>(new CharSequenceComparator(), QuicksortPivotSelector.RANDOM);
