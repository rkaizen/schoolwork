...
public class SuffixArray {
    ...
    public void build() {
        ...
        double time = Util.printTiming("Sorting suffix array", () -> {
            ...
            MultiKeyQuicksort<Integer> mkq = new MultiKeyQuicksort<>(counting, QuicksortPivotSelector.RANDOM);
            mkq.sort(sortedSuffixStarts, 0, sortedSuffixStarts.size(), 0);

            /*Quicksort<Integer> quicksort=new Quicksort<>(counting,QuicksortPivotSelector.RANDOM);
            quicksort.sort(sortedSuffixStarts);*/

            /*InsertionSort<Integer>insertion=new InsertionSort<>(counting);
            insertion.sort(sortedSuffixStarts);*/
    ...
    public List<String> sortedSuffixes(int maxLength) {
        return new AbstractList<String>() {
            ...
            public String get(int i) {
                ...
                return text.substring(start, start + maxLength); //wrapper that allows delimitation of returned get length based on max length argument
            }
        };
    }
    ...
    public void searchForKey(String searchKey, int maxNumMatches, int context, boolean trimLines) {
        ...
        Util.printTimingNL("Searching for \"" + searchKey + "\"", () -> {
            ...
            } else if ((endInterval - startInterval) - maxNumMatches > 0) {

                oldEnd = endInterval;

                endInterval = startInterval + maxNumMatches;
            ...
            if(!(oldEnd == 0)){
                System.out.println("[" + ((oldEnd - startInterval) - maxNumMatches) + " matches omitted]");
            }
    ...
    public static void main(String[] args) throws IOException {
        SuffixArray suffixArray = new SuffixArray("texts/bnc-miniscule.txt");
        suffixArray.build();
        suffixArray.writeToDisk();
        suffixArray.readFromDisk();
        suffixArray.searchForKey("hello", 1, 10, false);
    }
}
